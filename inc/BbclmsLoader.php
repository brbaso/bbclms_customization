...
if( !class_exists('BbclmsLoader') ):
    ...
    class BbclmsLoader{
		...
		private function define_admin_hooks() {			
			...
			$actions_to_add =[			
				//add category filter to events admin list
				'restrict_manage_posts' => [					
					[$plugin_functions, 'bbclms_events_category_filter']
				],
				// register Buddy Panel menu for Trainer
				'after_setup_theme' => [					
					[$plugin_functions, 'bbclms_register_bptrainer_menu']
				],
								
				// remove admin bar profile 
				'admin_bar_menu' => [					
					[$plugin_functions, 'bbclms_admin_bar_profile_remove', 999]
				],
				
				// remove admin menu pages 
				//add_action( 'admin_menu', 'bbclms_remove_meta_boxes()' );	
				'admin_menu' => [					
					[$plugin_functions, 'bbclms_remove_menu_pages'],
					[$plugin_functions, 'bbclms_remove_meta_boxes']
				],

				//add_action("add_meta_boxes", "bbclms_add_assignment_update_meta_box");
				'add_meta_boxes' => [					
					[$plugin_functions, 'bbclms_add_assignment_update_meta_box'],					
				],
				
				// block admin areas for certain users
				'admin_init' => [					
					[$plugin_functions, 'bbclms_blockusers_admin_areas']
				],
							
				// admin scripts
				'admin_enqueue_scripts' =>[
					[$plugin_functions, 'bbclms_admin_enqueue_styles'],
					[$plugin_functions, 'bbclms_admin_enqueue_scripts']
				],
				
				// bbclms_students_to_courses_groups_csv
				'wp_ajax_bbclms_students_to_courses_groups_csv' => [
					[$plugin_ajax, 'bbclms_students_to_courses_groups_csv']
				],
				
				// bbclms_groups_to_courses_csv
				'wp_ajax_bbclms_groups_to_courses_csv' => [
					[$plugin_ajax, 'bbclms_groups_to_courses_csv']
				],
				
				// bbclms_trainers_to_groups_csv
				'wp_ajax_bbclms_trainers_to_groups_csv' => [
					[$plugin_ajax, 'bbclms_trainers_to_groups_csv']
				],
				
				// bbclms_list_import_user_logs
				'wp_ajax_bbclms_list_import_user_logs' => [
					[$plugin_ajax, 'bbclms_list_import_user_logs']
				],
				
				// delete_log_files
				'wp_ajax_bbclms_delete_log_files' => [
					[$plugin_ajax, 'bbclms_delete_log_files']
				],
		...
		private function define_public_hooks() {
			...
			$plugin_db  = new BbclmsDB();
			$plugin_ajax = new BbclmsAjax( $plugin_db );
			//$plugin_csvimport = $this -> csvimport ;
			$plugin_this = $this;
			...
			$actions_to_add =[
				...
				// register widget
				'widgets_init' => [
					[$plugin_functions, 'bbclms_register_widgets']
				],
				
				// wp scripts
				'wp_enqueue_scripts' =>[
					[$plugin_functions, 'bbclms_enqueue_styles'],
					[$plugin_functions, 'bbclms_enqueue_scripts']
				],
				
				//add_action( 'bp_members_directory_member_types', array($this, 'bp_learndash_members_directory') );				
				'bp_members_directory_member_types' => [					
					[$plugin_functions, 'bclms_bp_learndash_members_directory']
				],
				
				//add_action( 'bp_actions', 'bbclms_rename_profile_tabs' );
				'bp_actions' => [					
					[$plugin_functions, 'bbclms_rename_profile_tabs']
				],
				
				//add_action( 'wp_head', 'bbclms_change_course_social_group_navigation',9 );
				'wp_head' => [					
					[$plugin_functions, 'bbclms_change_course_social_group_navigation',9]
				],
				
				// add_action( 'delete_user', 'bbclms_clear_course_access_delete_user' );
				'delete_user' => [
					[$plugin_functions, 'bbclms_clear_course_access_delete_user']
				],
				
				//add_action( 'learndash_assignment_uploaded', 'bbclms_learndash_assignment_uploaded' );
				'learndash_assignment_uploaded' => [
					[$plugin_functions, 'bbclms_learndash_assignment_uploaded']
				],
				
				//  add_action( 'delete_post', 'bbclms_clear_course_groups_delete_group', 10 );
				'delete_post' => [
					[$plugin_functions, 'bbclms_clear_course_groups_delete_group', 10]
				],
				
				// make sure CRON is called whenever WordPress loads
				//add_action('wp', 'bbclms_cronstarter_activation');
				//add_action('wp', 'bbclms_cron_csvimport_activation');
				'wp' => [
					//[$plugin_functions, 'bbclms_cronstarter_activation'],
					[$plugin_functions, 'bbclms_cron_csvimport_activation']
				],
				
				//// hook function(testmail cron testing) onto our scheduled event: mycronjob_csvimport
				// add_action ('mycronjob', 'bbclms_testmail_repeat_function');
				/*'mycronjob' => [
					[$plugin_functions, 'bbclms_testmail_repeat_function']
				],
				*/
				
				'wp_ajax_bbclms_contact_trainer_ajax' => [
					[$plugin_ajax, 'bbclms_contact_trainer_ajax']
				],
				
				'wp_ajax_nopriv_bbclms_contact_trainer_ajax' => [
					[$plugin_ajax, 'bbclms_contact_trainer_ajax']
				]
			];	
			...
			$filters_to_add =[
				...
				// add_filter( 'cron_schedules', 'bbclms_cron_add_three_minutes' );
				// add_filter( 'cron_schedules', 'bbclms_cron_add_half_hour' );
				'cron_schedules' => [					
					//[$plugin_functions, 'bbclms_cron_add_three_minutes'],
					[$plugin_functions, 'bbclms_cron_add_half_hour']
				],
